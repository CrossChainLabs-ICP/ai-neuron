...
module {
    ...
    func execute(state : S.State) : CommandHandler.Execute {
        func (_client : Sdk.OpenChat.Client, context : Sdk.Command.Context) : async Sdk.Command.Result {
            let n = Sdk.Command.Arg.maybeInt(context.command, "n") |> Option.get(_, 10) |> Int.abs(_);
            ...
            let text = "Subscribing to AI Neuron";                    
    ...
    func definition() : Sdk.Definition.Command {
        {
            name = "subscribe";
            description = ?"subscribe to AI Neuron Reports";
            placeholder = null;
            params = [{
                name = "n";
                description = ?"The delay in seconds between pings or 10 if undefined";
                placeholder = null;
                required = false;
