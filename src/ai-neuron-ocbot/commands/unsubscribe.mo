...
module {
    ...
    func execute(state : S.State) : CommandHandler.Execute {
        func (_client : Sdk.OpenChat.Client, context : Sdk.Command.Context) : async Sdk.Command.Result {
            ...
            let message = CommandResponse.EphemeralMessageBuilder(#Text { text = "Unsubscribe from AI Neuron Reports" }, chatDetails.message_id).build();
    ...
    func definition() : Sdk.Definition.Command {
        {
            name = "unsubscribe";
            description = ?"Unsubscribe from AI Neuron Reports";
            placeholder = null;
            params = [];
            permissions = {
                community = [];
